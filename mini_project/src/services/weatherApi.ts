import { GeocodeResult, CurrentWeather, DailyForecast, WeatherData } from '../types/weather';
...
export async function fetchWeatherForCoords(lat: number, lon: number): Promise<WeatherData | null> {
  ...
  if (!res.ok) return null;
  const data = await res.json();
  const location = { name: data?.timezone || 'Unknown', latitude: lat, longitude: lon } as GeocodeResult;
